...
Most of these symbols will not be initially exported as public symbols, meaning that only internal su-mbodules will be able to use them. As the need for external symbols arises, they will be gradually exposed under `tf.types`.

The initial type hierarchy is focused on V2 symbols; we expect few places where these symbols would not be compatible with V1 code, though.
