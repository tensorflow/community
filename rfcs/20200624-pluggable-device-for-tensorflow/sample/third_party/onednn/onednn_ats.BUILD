...
template_rule(
    ...
    substitutions = {
        ...
        "@DNNL_VERSION_HASH@": "2594510ba9d22c0056cf9deef39a56c4de8215dc",
    },
)
...
template_rule(
    ...
    substitutions = {
        "@KER_LIST_EXTERN@": "extern const char *gen12hp_systolic_gemm_copy_kernel[]; \n \
extern const char *gen12lp_gemm_nocopy_scale_x8x8s32_kernel[]; \
...
extern const char *gen12hp_1x1_conv_fwd_data_kernel[]; \
extern const char *gen12hp_conv_bwd_data_kernel[]; \
extern const char *gen12hp_conv_bwd_wei_bf16_kernel[]; \
extern const char *gen12hp_conv_bwd_wei_bf16_split_bar_kernel[]; \
extern const char *gen12hp_conv_fwd_data_kernel[]; \
extern const char *gen12hp_conv_fwd_data_ow_block_kernel[]; \
extern const char *gen12hp_reduction_kernel[]; \
extern const char *gen12hp_wei_convert_f32_to_bf16_kernel[]; \
extern const char *gen12hp_wei_f32_zero_init_kernel[]; \
...
extern const char *gen9_sum_kernel[]; \
...
extern const char *ref_prelu_kernel[]; \
...
extern const char *simple_sum_kernel[]; ",
        "@KER_LIST_ENTRIES@": "{ \"gen12hp_systolic_gemm_copy\", gen12hp_systolic_gemm_copy_kernel }, \n \
{ \"gen12hp_systolic_gemm_copy\", gen12hp_systolic_gemm_copy_kernel },\
{ \"gen12hp_systolic_gemm_copy\", gen12hp_systolic_gemm_copy_kernel },\
{ \"gen12hp_systolic_gemm_copy\", gen12hp_systolic_gemm_copy_kernel },\
{ \"gen12hp_systolic_gemm_copy\", gen12hp_systolic_gemm_copy_kernel },\
{ \"gen12hp_systolic_gemm_copy\", gen12hp_systolic_gemm_copy_kernel },\
{ \"gen12lp_gemm_scale_x8x8s32\", gen12lp_gemm_nocopy_scale_x8x8s32_kernel },\
...
{ \"gen12hp_1x1_conv_fwd\", gen12hp_1x1_conv_fwd_data_kernel },\
{ \"gen12hp_conv_bwd_data\", gen12hp_conv_bwd_data_kernel },\
{ \"gen12hp_conv_bwd_wei_bf16\", gen12hp_conv_bwd_wei_bf16_kernel },\
{ \"gen12hp_conv_bwd_wei_bf16_split_bar\", gen12hp_conv_bwd_wei_bf16_split_bar_kernel },\
{ \"gen12hp_conv_fwd\", gen12hp_conv_fwd_data_kernel },\
{ \"gen12hp_conv_fwd_ow_block\", gen12hp_conv_fwd_data_ow_block_kernel },\
{ \"gen12hp_initial_reduce\", gen12hp_reduction_kernel },\
{ \"gen12hp_final_reduce\", gen12hp_reduction_kernel },\
{ \"gen12hp_wei_convert_f32_to_bf16\", gen12hp_wei_convert_f32_to_bf16_kernel },\
{ \"gen12hp_wei_f32_zero_init\", gen12hp_wei_f32_zero_init_kernel },\
...
{ \"ref_conv_bwd_data\", ref_conv_bwd_data_u8s8s32x_kernel },\
{ \"ref_conv_fwd\", ref_conv_fwd_data_u8s8s32x_kernel },\
...
{ \"ref_prelu_fwd\", ref_prelu_kernel },\
{ \"ref_prelu_bwd\", ref_prelu_kernel },\
...
genrule(
    name = "mkl_dnn_genrule",
    outs = [
        "gemm_inner_product_kernel.cpp",
        "gemm_post_ops_inner_product_kernel.cpp",
        "gen12hp_1x1_conv_fwd_data_kernel.cpp",
        "gen12hp_conv_bwd_data_kernel.cpp",
        "gen12hp_conv_bwd_wei_bf16_kernel.cpp",
        "gen12hp_conv_bwd_wei_bf16_split_bar_kernel.cpp",
        "gen12hp_conv_fwd_data_kernel.cpp",
        "gen12hp_conv_fwd_data_ow_block_kernel.cpp",
        "gen12hp_reduction_kernel.cpp",
        "gen12hp_systolic_gemm_copy_kernel.cpp",
        "gen12hp_wei_convert_f32_to_bf16_kernel.cpp",
        "gen12hp_wei_f32_zero_init_kernel.cpp",
        ...
        "ref_conv_bwd_data_u8s8s32x_kernel.cpp",
        "ref_conv_fwd_data_u8s8s32x_kernel.cpp",
    ...
    cmd = "PROJECT_SOURCE_DIR=`pwd` && " +
          "MKLDNN_SOURCE_DIR=$$PROJECT_SOURCE_DIR/external/onednn_ats && " +
          "KER_GEN_DIR=kernel_gen_dir && " +
          "cd $$MKLDNN_SOURCE_DIR/src/gpu/ocl && " +
          "mkdir -p $$KER_GEN_DIR &&" +
          "KER_GEN_DIR=$$MKLDNN_SOURCE_DIR/src/gpu/ocl/$$KER_GEN_DIR &&" +
          "KER_INC_DIR=$$MKLDNN_SOURCE_DIR/src  && " +
          "for file in `find -iname \"*.cl\"`; " +
          "do KER_FILE=$$file; DCPP_FILE=`echo $${file##*/}`; DCPP_FILE=$${DCPP_FILE%%.cl}_kernel.cpp; " +
          "cmake -DKER_FILE=$$KER_FILE -DGEN_FILE=$$DCPP_FILE -DKER_INC_DIR=$$KER_INC_DIR -P $$MKLDNN_SOURCE_DIR/cmake/gen_gpu_kernel.cmake; " +
          "mv $$DCPP_FILE $$KER_GEN_DIR; " +
          "done && " +
          "cd $$KER_GEN_DIR && " +
          "output_path=$$PROJECT_SOURCE_DIR/bazel-out/k8-opt/bin/external/onednn_ats/ && " +
          "[[ -d $$output_path ]] && cp -f *_kernel.cpp $$output_path; " +
          "output_path=$$PROJECT_SOURCE_DIR/bazel-out/host/external/onednn_ats/ && " +
          "[[ -d $$output_path ]] && cp -f *_kernel.cpp $$output_path; " +
          "output_path=$$PROJECT_SOURCE_DIR/execroot/org_tensorflow/bazel-out/k8-dbg/bin/external/onednn_ats/ && " +
          "[[ -d $$output_path ]] && cp -f *_kernel.cpp $$output_path; " +
          "rm -rf $$KER_GEN_DIR",
)
