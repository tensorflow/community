...
def set_dpcpp_toolkit_path(environ_cp):
  ...
  dpcpp_toolkit_path = prompt_loop_or_load_from_env(
      ...
      ask_for_var=(
          'Please specify the location where DPC++ for SYCL %s is '
...
def main():
  ...
  current_bazel_version = check_bazel_version('3.1.0', '3.1.0')
...
#
  set_action_env_var(environ_cp, 'TF_NEED_DPCPP', 'DPC++', True)
  if environ_cp.get('TF_NEED_DPCPP') == '1':
    #set_action_env_var(environ_cp, 'TF_NEED_COMPUTECPP', 'ComputeCPP compiler', True)
    set_action_env_var(environ_cp, 'TF_NEED_DPCPP_BUILD', 'DPC++ compiler', True)
    if environ_cp.get('TF_NEED_DPCPP_BUILD') == '1':
      set_dpcpp_toolkit_path(environ_cp)
      gcc_toolchain_path = set_gcc_toolchain_path(environ_cp)
      set_gcc_toolchain_target(environ_cp, gcc_toolchain_path)
    else:
      raise UserInputError('Must configure the DPC++ compiler.')
  ...
  if gpu_platform_count >= 2:
    raise UserInputError('SYCL / DPC++ are mututally exclusive. '
                         'At most 1 GPU platform can be configured.')
