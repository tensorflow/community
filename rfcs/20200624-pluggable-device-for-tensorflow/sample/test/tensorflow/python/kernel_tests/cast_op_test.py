...
class GpuCastOpTest(test.TestCase):
  ...
  def _cast(self, x, dtype, use_gpu=True):
  ...
  def testSmallValues(self):
    ...
    self._testAll(
        np.array([
            0, -1, 1, -f4.resolution, f4.resolution
        ]))
  ...
  def _compare(self, x, dst_dtype, expected, use_gpu=True):
