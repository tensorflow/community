#ifndef TENSORFLOW_PLUGIN_SRC_XPU_CORE_UTIL_LOGGING_H_
#define TENSORFLOW_PLUGIN_SRC_XPU_CORE_UTIL_LOGGING_H_
...
#define _TF_LOG_INFO \
  ::intel_plugin::internal::LogMessage(__FILE__, __LINE__, ::intel_plugin::INFO)
#define _TF_LOG_WARNING                                    \
  ::intel_plugin::internal::LogMessage(__FILE__, __LINE__, \
                                       ::intel_plugin::WARNING)
#define _TF_LOG_ERROR                                      \
  ::intel_plugin::internal::LogMessage(__FILE__, __LINE__, \
                                       ::intel_plugin::ERROR)
#define _TF_LOG_FATAL \
  ::intel_plugin::internal::LogMessageFatal(__FILE__, __LINE__)
...
#define VLOG_IS_ON(lvl)                                                       \
  (([](int level, const char* fname) {                                        \
    static const bool vmodule_activated =                                     \
        ::intel_plugin::internal::LogMessage::VmoduleActivated(fname, level); \
...
#define VLOG(level)                                                \
  ...
  ? (void)0                                                        \
  : ::intel_plugin::internal::Voidifier() &                        \
          ::intel_plugin::internal::LogMessage(__FILE__, __LINE__, \
                                               intel_plugin::INFO)
...
#define DVLOG(verbose_level)           \
  ...
  ::intel_plugin::internal::LogMessageNull()
#endif
...
#define LOGGING_INTERNAL_STATEFUL_CONDITION(kind, condition, arg)   \
  for (bool logging_internal_stateful_condition_do_log(condition);  \
    ...
    for (static ::intel_plugin::internal::Log##kind##State          \
...
#define TF_DEFINE_CHECK_OP_IMPL(name, op)                                   \
  ...
  inline string* name##Impl(const T1& v1, const T2& v2,                     \
    ...
    if (TF_PREDICT_TRUE(v1 op v2))                                          \
      ...
      return ::intel_plugin::internal::MakeCheckOpString(v1, v2, exprtext); \
  }                                                                         \
...
#define CHECK_OP_LOG(name, op, val1, val2)                       \
  while (::intel_plugin::internal::CheckOpString _result{        \
      ::intel_plugin::internal::name##Impl(                      \
          ::intel_plugin::internal::GetReferenceableValue(val1), \
          ::intel_plugin::internal::GetReferenceableValue(val2), \
          #val1 " " #op " " #val2)})                             \
  ::intel_plugin::internal::LogMessageFatal(__FILE__, __LINE__)  \
      << *(_result.str_)
...
#define CHECK_NOTNULL(val)                                   \
  ::intel_plugin::internal::CheckNotNull(__FILE__, __LINE__, \
...
#endif  // TENSORFLOW_PLUGIN_SRC_UTIL_LOGGING_H_
