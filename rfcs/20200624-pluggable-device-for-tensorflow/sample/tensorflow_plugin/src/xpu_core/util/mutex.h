#ifndef TENSORFLOW_PLUGIN_SRC_XPU_CORE_UTIL_MUTEX_H_
#define TENSORFLOW_PLUGIN_SRC_XPU_CORE_UTIL_MUTEX_H_
...
#include "tensorflow_plugin/src/xpu_core/util/thread_annotations.h"
...
#include "tensorflow_plugin/src/xpu_core/util/mutex_data.h"
...
// Mimic a subset of the std::unique_lock<intel_plugin::mutex> functionality.
class TF_SCOPED_LOCKABLE mutex_lock {
 public:
  typedef ::intel_plugin::mutex mutex_type;
...
// Mimic a subset of the std::shared_lock<intel_plugin::mutex> functionality.
...
std::cv_status condition_variable::wait_for(
  ...
  return intel_plugin::internal::wait_until_system_clock(
      &this->cv_, &lock.mutex()->mu_, std::chrono::system_clock::now() + dur);
}

}  // namespace intel_plugin

#endif  // TENSORFLOW_PLUGIN_SRC_XPU_CORE_UTIL_MUTEX_H_
