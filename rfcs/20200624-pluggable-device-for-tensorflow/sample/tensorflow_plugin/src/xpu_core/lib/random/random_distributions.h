#ifndef TENSORFLOW_PLUGIN_SRC_XPU_CORE_LIB_RANDOM_RANDOM_DISTRIBUTIONS_H_
#define TENSORFLOW_PLUGIN_SRC_XPU_CORE_LIB_RANDOM_RANDOM_DISTRIBUTIONS_H_
...
// Helper function to convert a 16-bit integer to a Eigen::bfloat16 between
// [0..1).
PHILOX_DEVICE_INLINE Eigen::bfloat16 Uint16ToGfloat16(uint16 x);
...
template <class Generator>
class UniformDistribution<Generator, Eigen::bfloat16> {
 public:
  // The number of elements that will be returned.
  static constexpr int kResultElementCount = Generator::kResultElementCount;
  // Cost of generation of a single element (in cycles).
...
template <class Generator>
class NormalDistribution<Generator, Eigen::bfloat16> {
 public:
  // The number of elements that will be returned.
  static constexpr int kResultElementCount = Generator::kResultElementCount;
  // Cost of generation of a single element (in cycles).
  ...
  ResultType operator()(Generator* gen) {
    ...
    for (int i = 0; i < kResultElementCount; i += 2) {
      ...
      BoxMullerFloat(sample[i], sample[i + 1], &f[0], &f[1]);
      result[i] = Eigen::bfloat16(f[0]);
      result[i + 1] = Eigen::bfloat16(f[1]);
    }
    return result;
  }
};
...
template <class SingleSampleGenerator>
class TruncatedNormalDistribution<SingleSampleGenerator, Eigen::bfloat16> {
 public:
  // The number of elements that will be returned.
  static constexpr int kResultElementCount =
      SingleSampleGenerator::kNativeElementCount;
  ...
  ResultType operator()(SingleSampleGenerator* gen) {
    ...
    while (true) {
      ...
      if (Eigen::numext::abs(f[0]) < kTruncateValue) {
        results[index++] = Eigen::bfloat16(f[0]);
        if (index >= kResultElementCount) {
          return results;
        }
      }
      if (Eigen::numext::abs(f[1]) < kTruncateValue) {
        results[index++] = Eigen::bfloat16(f[1]);
        if (index >= kResultElementCount) {
          return results;
        }
      }
...
#if defined(__SYCL_DEVICE_ONLY__) || !defined(__linux__)
...
PHILOX_DEVICE_INLINE Eigen::half Uint16ToHalf(uint16 x) {
  ...
  Eigen::half result;
  result = *((Eigen::half*)&val);
  // result.x = val;
  return result - Eigen::half(1.0);
}

// Helper function to convert an 16-bit integer to a Eigen::bfloat16 between
// [0..1). This can create a uniform distribution of values between [0..1).
PHILOX_DEVICE_INLINE Eigen::bfloat16 Uint16ToGfloat16(uint16 x) {
  ...
  return result - Eigen::bfloat16(1.0);
}
...
#endif  // TENSORFLOW_PLUGIN_SRC_XPU_CORE_LIB_RANDOM_RANDOM_DISTRIBUTIONS_H_
