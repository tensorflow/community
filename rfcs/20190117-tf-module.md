...
class Module(six.with_metaclass(ModuleMetaclass, tf.checkpoint.Checkpointable)):
  ...
  def owned_variables(self):
    """Returns a collection of variables that are attributes of the current module.
...
-   `module.owned_variables` - All variables from this module but not its
    dependent modules.
-   `module.owned_trainable_variables` - All owned trainable variables.
...
-   `module.owned_submodules` - All immediate child modules for this module.
...
# Sequential doesn't own any variables.
assert not set(mlp.owned_variables)
...
# Sequential depends directly on it's layers so these are it's children.
assert set(mlp.owned_submodules) == {mlp.layers[0], mlp.layers[2]}
