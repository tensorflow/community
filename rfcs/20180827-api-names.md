...
**tf.debugging** - ops helpful for debugging, such as asserts.
...
In total, we propose to remove 230 endpoints, including 176 endpoints in the root namespace.

See full list of endpoints we want to remove in [Appendix 2: Deprecated
Endpoints](#appendix-2).


# Impact

Browsing for symbols should become easier. For e.g. page for <span style="color:#38761d;">tf.math</span> namespace should display all math functions that TensorFlow provides. Similarly, <span style="color:#38761d;">tf.manip</span> namespace page should display all available tensor manipulation ops.
...
# Appendix 1: Additional Endpoints {#appendix-1}
...
| tf.FixedLengthRecordReader | tf.io.FixedLengthRecordReader |
...
| tf.IdentityReader | tf.io.IdentityReader |
...
| tf.ReaderBase | tf.io.ReaderBase |
...
| tf.TFRecordReader | tf.io.TFRecordReader |
| tf.TextLineReader | tf.io.TextLineReader |
| tf.VERSION | tf.version.VERSION |
| tf.VarLenFeature | tf.io.VarLenFeature |
| tf.WholeFileReader | tf.io.WholeFileReader |
| tf.abs | tf.math.abs |
...
| tf.setdiff1d | tf.manip.setdiff1d |
...
| tf.train.queue_runner.QueueRunner | tf.io.QueueRunner |
| tf.train.queue_runner.add_queue_runner | tf.io.add_queue_runner |
| tf.train.queue_runner.start_queue_runners | tf.io.start_queue_runners |
| tf.train.start_queue_runners | tf.io.start_queue_runners |
...
# Appendix 2: Deprecated Endpoints {#appendix-2}
...
| tf.FixedLengthRecordReader | replace with tf.io.FixedLengthRecordReader |
...
| tf.IdentityReader | replace with tf.io.IdentityReader |
...
| tf.ReaderBase | replace with tf.io.ReaderBase |
...
| tf.TextLineReader | replace with tf.io.TextLineReader |
| tf.VERSION | replace with tf.version.VERSION |
| tf.WholeFileReader | replace with tf.io.WholeFileReader |
...
| tf.train.queue_runner.QueueRunner | replace with tf.io.QueueRunner |
| tf.train.queue_runner.add_queue_runner | replace with tf.io.add_queue_runner |
| tf.train.queue_runner.start_queue_runners | replace with tf.io.start_queue_runners |
