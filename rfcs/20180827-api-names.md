...
**tf.layers** - replace with `tf.keras.losses`.
...
*   Remove all endpoints from `tf.metrics`, `tf.losses` and `tf.layers`.
*   Keeps most symbols in `tf.manip` in root namespace.

In total, we propose to remove 205 endpoints, including 169 endpoints in the root namespace.

See the list of endpoints we want to remove in *Appendix 2: Deprecated
Endpoints*. Note: the list in the appendix does not include endpoints under `tf.layers` and
`tf.logging` since entire contents of these modules will be deprecated. So, we
don't need to list these endpoints individually.
...
`tf.layers` (See *Deprecated namespaces* section above).
